...
def sub_load_data(data, img_size, aug):
    ...
    img = imageio.imread(dataset+'img_files/'+img_name+'.bmp', mode='L')


    try:
        seg = imageio.imread(dataset + 'seg_files/' + img_name + '.bmp', mode='L')
    except:
        seg = np.ones_like(img)

    try:
        ali = imageio.imread(dataset+'ori_files/'+img_name+'.jpg', mode='L')
    except:
...
def get_maximum_img_size_and_names(dataset, sample_rate=None, max_size=None):
    ...
    for folder, rate in zip(dataset, sample_rate):
        ...
        img_size.append(np.array(imageio.imread(folder + 'img_files/' + img_name_t[0] + '.png', pilmode='L').shape))
